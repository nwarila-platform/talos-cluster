#!/usr/bin/env python3
"""Fail if retained kubelet-csr-approver addon values drift from Flux."""
...
DEFAULT_ADDON_VALUES = Path("addons/kubelet-csr-approver/values.yaml")
DEFAULT_HELMRELEASE = Path(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare addons/kubelet-csr-approver/values.yaml with the Flux "
            "kubelet-csr-approver HelmRelease spec.values mapping after YAML "
            "normalization."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"ERROR: failed to load kubelet-csr-approver values inputs: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if addon_values == helmrelease_values:
        print(
            f"kubelet-csr-approver values match: {args.addon_values} == "
    ...
    print(
        "ERROR: retained kubelet-csr-approver addon values drift from Flux "
