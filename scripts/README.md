...
| `check-vault-jwt-github-invariants.py` (510) | GitHub-OIDC config, generation-aware health, exact-host egress, and every future JWT role retain the ratified fail-closed contract | CRD/kstatus validates neither the cross-resource security semantics nor Go-duration bounds → **NO** | **KEEP** — executable D9/ADR-0031 contract with field-focused negative fixtures |
