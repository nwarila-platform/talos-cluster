...
| `rendered-inventory.py` (1029) | Fail-closed shared inventory: select and validate the effective Flux `vault-config-managed` object from the root render, then render its applied managed set with Flux-compatible load semantics | Kustomize renders resources but does not select or validate the controlling Flux object or enforce this directory's containment contract → **NO** | **KEEP** — one authority shared by the three Vault-config guards |
