...
| `check-tunnel-isolation.py` (1939) | Enforces exact connector/proxy/Flux pairing; closed whole-object connector and tenant policy, Namespace, and route inventories across local renders and the root aggregate Flux applies, plus proxy closure in each applied child render; exact content-hashed root/apps/tenant build indexes; every Flux Kustomization enumerated by resolved `spec.path` with forbidden build modifiers absent; connector/proxy host-namespace posture; Kyverno Deny/Fail tenant-match posture; the closed tunnel-to-tenant ownership and class/zone registrations; case-insensitive placeholder rejection; and the tree-wide retired-boolean ban | nothing correlates Flux build inputs and whole rendered policy objects with connector routes, tenant ownership, and CEL class maps; a widened policy or cross-tunnel route could silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
