...
| `check-vault-config-operator-bootstrap-invariants.py` (560) | The operator's own identity + break-glass `vault-admin` are never self-managed/self-escalating (ADR-0028 bootstrap paradox) | none → **NO** | **KEEP** — highest-stakes anti-self-escalation invariant |
| `check-vault-jwt-github-invariants.py` (529) | GitHub-OIDC config, generation-aware health, exact-host egress, and every recursively discovered managed JWT role retain the ratified contract | CRD/kstatus validates neither the cross-resource security semantics nor Go-duration bounds → **NO** | **KEEP** — executable D9/ADR-0031 contract with field-focused negative fixtures |
| `check-vault-config-reference-safety.py` (502) | Every Vault-object reference (role→policy, issuer→mount, …) resolves to an in-git provider (S7 prune safety) | cross-CRD semantic refs are opaque to Flux/kustomize → **NO** | **KEEP** |
