...
| `check-tunnel-isolation.py` (1155) | Enforces exact 1:1 connector/proxy/Flux inventory, closed-world policy rules, tenant ownership, routes, exact same-zone canary reservations, and the fixed class-to-tier map; each overlay declaration must agree and each connector render must have its canonical Access posture | no upstream validator correlates rendered policies and routes with tenant ownership, same-zone reservations, and a repository-owned tier classification; a widened selector or lying tier could republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
