...
| `check-sigstore-pin-verification.py` (218) | Daily: the guard-pinned policy is present+Ready and every first-party sig still verifies against the pinned keys (rotation drift for the current `[Deny]`/`Fail` merged IVP) | Consumes Kyverno's own PolicyReport ground truth (the *correct* native use) | **KEEP** — thin, honest, reads native truth instead of rebuilding cosign/TUF |
...
| `rendered-inventory.py` (1124) | Fail-closed shared inventory with parallel modes: select and validate the effective Flux `vault-config-managed` object from the root render, then render its applied managed set with Flux-compatible load semantics; or discover and classify Flux owners to a fixed point over the reachable unmodified in-repository subset, render only that desired-build subset with explicit-null rejection, and bound incomplete frontiers in `reach_limits` (a modified owner is partially searched whenever its path was rendered; repository desired build, not live state) | Kustomize renders resources but does not select or validate the controlling Flux object or enforce this directory's containment contract → **NO** | **KEEP** — one authority shared by the three Vault-config guards |
| `check-vault-config-operator-bootstrap-invariants.py` (627) | Checks the rendered applied target plus stricter filesystem allow-set/prohibition inputs for protected identities, and checks bootstrap grants against the derived managed names and JWT paths | none → **NO** | **KEEP** — highest-stakes anti-self-escalation invariant; remaining authored assertions are TD-0019 |
| `check-vault-jwt-github-invariants.py` (550) | The rendered applied GitHub-OIDC config, health contract, and managed JWT roles retain the ratified contract; the exact-host egress source remains pinned | CRD/kstatus validates neither the cross-resource security semantics nor Go-duration bounds → **NO** | **KEEP** — executable D9/ADR-0031 contract with field-focused negative fixtures |
| `check-vault-config-reference-safety.py` (538) | Checks managed providers and managed-role edges from the rendered applied inventory plus the S7 prune-safety edges from filesystem-discovered structured, capture-only, and pinned consumers | cross-CRD semantic refs are opaque to Flux/kustomize → **NO** | **KEEP** — all-Flux consumer discovery and authored-value residuals are TD-0018/TD-0019 |
...
| `check-tunnel-isolation.py` (1506) | Enforces exact 1:1 connector/proxy/Flux inventory; whole-object equality for every rendered `CiliumNetworkPolicy`, `CiliumClusterwideNetworkPolicy`, and `NetworkPolicy` in every tunnel overlay and tenant render; the closed tunnel-to-tenant map with tenant/org ownership; tunnel-local route targets; unique class registration; exact same-zone canary reservations; the fixed class-to-tier map and canonical rendered Access posture; tunnel Flux children with no build-affecting keys; case-insensitive placeholder rejection; and the tree-wide retired-boolean ban | nothing correlates Flux build inputs and whole rendered policy objects with connector routes, tenant ownership, same-zone reservations, and repository-owned tier classification; a widened policy, cross-tunnel route, or lying tier could silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
| `register-tunnel-hostname.py` (581) | Converges mTLS-tier hostname association, Access domains, WAF, then DNS; removals delete DNS first, reconcile reads only labelled tenant namespaces, and closed reserved-name/DNS-ownership checks fail before opening a route | the four Cloudflare primitives are per-FQDN and outside Flux; safe sequencing and field-preserving updates require a purpose-built state machine → **NO** | **KEEP** — operator entry point and future reconciler core; the fake-client selftest proves ordered calls without touching the edge |
...
| `check-no-placeholder-leak.sh` (100) | No unresolved `placeholder` survives render (a tenant overlay missing a `replacements` block admits at Kyverno but breaks VSO at runtime) | kustomize renders unreplaced placeholders happily → **NO** | **KEEP** |
