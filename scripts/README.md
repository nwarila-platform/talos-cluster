...
| `rendered-inventory.py` (1107) | Fail-closed shared inventory with both parallel modes: select and validate the effective Flux `vault-config-managed` object from the root render, then render its applied managed set with Flux-compatible load semantics; or discover and classify Flux owners to a fixed point over the unmodified in-repository subset, render only that desired-build subset, and name the excluded modified/external owners in `reach_limits` with unmodified-owner `apply_semantics` (repository desired build, not live state) | Kustomize renders resources but does not select or validate the controlling Flux object or enforce this directory's containment contract → **NO** | **KEEP** — one authority shared by the three Vault-config guards |
