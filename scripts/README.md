...
| `check-tunnel-isolation.py` (448) | Renders every Cloudflare Tunnel overlay and checks what Flux applies: each connector, proxy, inherited tenant allow, and admission registration references only its own tunnel, no placeholder survives the shared components, and an unprotected class stays barred from a protected zone nested inside its own | nothing correlates a rendered CNP label value, a connector route table, and a CEL class map; a boolean opt-in would silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the only machine check on the protected/unprotected tunnel boundary |
| `register-tunnel-hostname.py` (317) | Converges the Cloudflare edge for the mTLS tunnel tier per hostname, in the only safe order (client-cert association, Access app domain, WAF entry, then DNS; reverse on removal) | every one of those primitives is per-FQDN and none is reachable from Git or Flux → **NO** | **KEEP** — the loop the scheduled reconciler runs; `--dry-run` prints the plan |
