...
| `check-tunnel-isolation.py` (1343) | Enforces exact 1:1 connector/proxy/Flux inventory; whole-object equality for every rendered `CiliumNetworkPolicy`, `CiliumClusterwideNetworkPolicy`, and `NetworkPolicy` in every tunnel overlay and tenant render; the closed tunnel-to-tenant map with tenant/org ownership; tunnel-local route targets; unique class registration; tunnel Flux children with no build-affecting keys; case-insensitive placeholder rejection; and the tree-wide retired-boolean ban | nothing correlates Flux build inputs and whole rendered policy objects with connector routes, tenant ownership, and CEL class maps; a widened policy or cross-tunnel route could silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
