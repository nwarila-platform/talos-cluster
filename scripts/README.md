...
| `check-tunnel-isolation.py` (2153) | Enforces exact connector/proxy/Flux pairing; closed whole-object connector and tenant policy, Namespace, and route inventories across local renders and the root aggregate Flux applies, plus proxy closure in each applied child render; exact content-hashed root/apps/tenant/Flux-system/Kyverno-policy build indexes; every Flux Kustomization present in the applied builds (root aggregate and each internal child render, to a fixed point), validated by resolved `spec.path` with forbidden protected-path build modifiers absent; connector/proxy host-namespace posture; rendered and authored Kyverno Deny/Fail tenant-match posture with exact policy identities; the closed tunnel-to-tenant ownership and class/zone registrations; case-insensitive placeholder rejection; and the tree-wide retired-boolean ban | nothing correlates Flux build inputs and whole rendered policy objects with connector routes, tenant ownership, and CEL class maps; a widened policy or cross-tunnel route could silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
