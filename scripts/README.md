...
| `check-tunnel-isolation.py` (993) | Enforces exact 1:1 connector/proxy/Flux inventory, closed-world rendered policy sets, exact allow-listed CNP rule shapes and ports, the closed tunnel-to-tenant map with tenant/org ownership, route containment, unique class registration, case-insensitive placeholder rejection, and the tree-wide retired-boolean ban | nothing correlates rendered policy objects and rules, connector routes, tenant ownership, and CEL class maps; a widened selector or boolean opt-in could silently republish an mTLS origin unauthenticated → **NO** | **KEEP** — the machine check on the protected/unprotected tunnel boundary |
