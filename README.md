...
- **GitOps:** Flux `v2.9.3` bootstraps from `clusters/talos-cluster/flux-system/` and reconciles app and tenant manifests from this repository.
...
|----------|---------|-------------|----------------|
| **TalosOS** | v1.13.6 | The operating system on each node. Secure, immutable, API-managed. | It's the foundation: every node runs this instead of Ubuntu, CentOS, etc. |
...
| **Flux** | v2.9.3 | Reconciles the Kubernetes manifests under `clusters/talos-cluster/`. | Keeps Git as the operational source of truth after bootstrap. |
