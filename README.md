...
- **Policy:** Kyverno `3.8.2` is reconciled by Flux. First-party image signatures for `ghcr.io/nwarila-platform/*`, `ghcr.io/nwarila/*`, and `ghcr.io/the-hero-wars-guys/*` are verified by one merged `verify-first-party` ImageValidatingPolicy currently canaried at `[Audit]`/`Ignore` (non-blocking); the steady-state follow-up target is `[Deny]`/`Fail`. Upstream Flux, Cilium, Kyverno, and VSO images remain audit-only pending a re-signing registry (TD-0001/TD-0002).
...
| **Kyverno** | 3.8.2 | Provides Kubernetes admission policy; verifies first-party image signatures through one merged `verify-first-party` ImageValidatingPolicy currently canaried at `[Audit]`/`Ignore` (non-blocking), with a follow-up target of `[Deny]`/`Fail`. Upstream families remain audit-only. | Gives the cluster a policy engine without requiring ad hoc manual admission checks. |
...
| **Image-signature admission** | Kyverno verifies container image signatures at admission; first-party images are cosign-signed and admission-verified by one merged `verify-first-party` ImageValidatingPolicy currently in a non-blocking `[Audit]`/`Ignore` canary. The follow-up steady state is `[Deny]`/`Fail`; upstream families are audited. |
