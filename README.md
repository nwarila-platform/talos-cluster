...
A real Talos Kubernetes homelab platform managed by Flux GitOps: first-party image signatures are verified at admission and fail-closed enforced (unsigned or unverified first-party images are denied, using offline signature verification with no online transparency-log dependency), upstream image families remain audit-only, and full topology is public by a documented threat-model decision.

## Overview

This is the source-of-truth repository for a real Talos Kubernetes homelab. Talos machine state is generated from `cluster/config.env` and `cluster/patches/`, while Kubernetes platform and application state is reconciled by Flux from `clusters/talos-cluster/`. Secrets have two paths: SOPS-encrypted Secret manifests in git for bootstrap and platform material, and Vault/Vault Secrets Operator runtime flows where those integrations are implemented. Admission policy verifies first-party image signatures and fail-closed enforces them — an unsigned or unverified first-party image is denied at admission, using offline signature verification (the embedded cosign bundle is checked against pinned Sigstore keys, with no online transparency-log query on the admission path) — while upstream Flux, Cilium, Kyverno, and VSO image families remain audit-only pending TD-0001/TD-0002. Disaster recovery is tiered: Stage-0 S3 holds rebuild-critical secrets only, daily in-cluster etcd and Vault snapshot jobs ship operational state to the Stage-1 Synology backup target, and restore is not accepted until drilled.
...
- **Policy:** Kyverno `3.8.2` is reconciled by Flux. First-party image signatures for `ghcr.io/nwarila-platform/*`, `ghcr.io/nwarila/*`, and `ghcr.io/the-hero-wars-guys/*` are verified at admission and fail-closed enforced — an unsigned or unverified first-party image is denied, using offline signature verification (the embedded cosign bundle is checked against pinned Sigstore keys, with no online transparency-log query on the admission path); upstream Flux, Cilium, Kyverno, and VSO images remain audit-only pending a re-signing registry (TD-0001/TD-0002).
...
| **Kubernetes** | v1.36.2 | The container orchestration platform. Manages all running applications. | It's the core: this is what makes the cluster a cluster. |
...
| **Kyverno** | 3.8.2 | Provides Kubernetes admission policy; verifies and fail-closed enforces first-party image signatures (unsigned or unverified first-party images are denied, using offline signature verification) while auditing upstream families. | Gives the cluster a policy engine without requiring ad hoc manual admission checks. |
...
| **Image-signature admission** | Kyverno verifies container image signatures at admission; first-party images are cosign-signed and admission-verified with fail-closed enforcement (unsigned or unverified first-party images are denied, using offline signature verification), while upstream families are audited. |
