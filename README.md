...
- **GitOps:** Flux `v2.9.1` bootstraps from `clusters/talos-cluster/flux-system/` and reconciles app and tenant manifests from this repository.
...
|----------|---------|-------------|----------------|
| **TalosOS** | v1.13.7 | The operating system on each node. Secure, immutable, API-managed. | It's the foundation: every node runs this instead of Ubuntu, CentOS, etc. |
...
| **Cilium** | 1.19.6 | Handles pod networking, replaces kube-proxy, and provides the Gateway API dataplane. | Without a CNI (Container Network Interface), containers on different nodes cannot talk to each other. |
...
| **Flux** | v2.9.1 | Reconciles the Kubernetes manifests under `clusters/talos-cluster/`. | Keeps Git as the operational source of truth after bootstrap. |
...
| **Image-signature admission** | Kyverno verifies container image signatures at admission; first-party images are cosign-signed and admission-enforced by one merged `verify-first-party` ImageValidatingPolicy at `[Deny]`/`Fail`; upstream families are audited. |
