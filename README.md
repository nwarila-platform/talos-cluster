...
- **Policy:** Kyverno `3.8.2` is reconciled by Flux. First-party image signatures for `ghcr.io/nwarila-platform/*`, `ghcr.io/nwarila/*`, and `ghcr.io/the-hero-wars-guys/*` are verified at admission with live fail-closed enforcement; as of 2026-07-18, a known Kyverno v1.18.2 handoff defect can intermittently deny signed images and its remedy is under diagnosis; upstream Flux, Cilium, Kyverno, and VSO images remain audit-only pending a re-signing registry (TD-0001/TD-0002).
...
| **Kyverno** | 3.8.2 | Provides Kubernetes admission policy; verifies first-party image signatures with live fail-closed enforcement. A known Kyverno v1.18.2 handoff defect can intermittently deny signed images; remedy under diagnosis. Upstream families remain audit-only. | Gives the cluster a policy engine without requiring ad hoc manual admission checks. |
...
| **Image-signature admission** | Kyverno verifies container image signatures at admission; first-party images are cosign-signed and admission-verified with live fail-closed enforcement. A known Kyverno v1.18.2 handoff defect can intermittently deny signed images; remedy under diagnosis. Upstream families are audited. |
