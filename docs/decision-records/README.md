...
| [0019](repo/0019-enable-tokenless-vault-generate-root.md) | Enable token-less Vault generate-root for recovery-key break-glass | Accepted | 2026-07-01 | Enable `enable_unauthenticated_access = ["generate-root"]` so restored token-less Vault snapshots can mint a fresh root from the recovery-key quorum. |
